...
ko.utils = new (function () {
    ...
    // Represent the known eventInfo types in a compact way, then at runtime transform it into a hash with eventInfo name as key (for fast lookup)
        ...
        registerEventHandler: function (element, eventType, handler) {
            ...
            if (!mustUseAttachEvent && typeof jQuery != "undefined") {
                if (isClickOnCheckableElement(element, eventType)) {
                    // For click events on checkboxes, jQuery interferes with the eventInfo handling in an awkward way:
                    // it toggles the element checked state *after* the click eventInfo handlers run, whereas native
                    // click events toggle the checked state *before* the eventInfo handler.
                    // Fix this by intecepting the handler and applying the correct checkedness before it runs.
                    var originalHandler = handler;
                    handler = function(eventInfo, eventData) {
                        var jQuerySuppliedCheckedState = this.checked;
                        if (eventData)
                            this.checked = eventData.checkedStateBeforeEvent !== true;
                        originalHandler.call(this, eventInfo);
                ...
                element.attachEvent("on" + eventType, function (eventInfo) {
                    handler.call(element, eventInfo);
                });
            else
        ...
        triggerEvent: function (element, eventType) {
            ...
            if (typeof jQuery != "undefined") {
                ...
                if (typeof element.dispatchEvent == "function") {
                    ...
                    var eventInfo = document.createEvent(eventCategory);
                    eventInfo.initEvent(eventType, true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, element);
                    element.dispatchEvent(eventInfo);
                }
                else
                    throw new Error("The supplied element doesn't support dispatchEvent");
            } else if (typeof element.fireEvent != "undefined") {
                // Unlike other browsers, IE doesn't change the checked state of checkboxes/radiobuttons when you trigger their "click" eventInfo
...
ko.subscribable['fn'] = {
    subscribe: function (callback, callbackTarget, eventInfo) {
        eventInfo = eventInfo || defaultEvent;
        var boundCallback = callbackTarget ? callback.bind(callbackTarget) : callback;

        var subscription = new ko.subscription(this, boundCallback, function () {
            ko.utils.arrayRemoveItem(this._subscriptions[eventInfo], subscription);
        }.bind(this));

        if (!this._subscriptions[eventInfo])
            this._subscriptions[eventInfo] = [];
        this._subscriptions[eventInfo].push(subscription);
        return subscription;
    },

    "notifySubscribers": function (valueToNotify, eventInfo) {
        eventInfo = eventInfo || defaultEvent;
        if (this._subscriptions[eventInfo]) {
            ko.utils.arrayForEach(this._subscriptions[eventInfo].slice(0), function (subscription) {
...
(function () {
    ...
    function applyBindingsToNodeInternal (node, bindings, viewModelOrBindingContext, bindingContextMayDifferFromDomParentElement) {
        ...
        // DOM eventInfo callbacks need to be able to access this changed data,
...
// e.g. click:handler instead of the usual full-length eventInfo:{click:handler}
...
ko.utils.arrayForEach(eventHandlersWithShortcuts, function(eventName) {
    ko.bindingHandlers[eventName] = {
        'init': function(element, valueAccessor, allBindingsAccessor, viewModel) {
            ...
            return ko.bindingHandlers['eventInfo']['init'].call(this, element, newValueAccessor, allBindingsAccessor, viewModel);
        }
    }
});


ko.bindingHandlers['eventInfo'] = {
    'init' : function (element, valueAccessor, allBindingsAccessor, viewModel) {
        var eventsToHandle = valueAccessor() || {};
        for(var eventNameOutsideClosure in eventsToHandle) {
            (function() {
                var eventName = eventNameOutsideClosure; // Separate variable to be captured by eventInfo handler closure
                if (typeof eventName == "string") {
                    ko.utils.registerEventHandler(element, eventName, function (eventInfo) {
                        ...
                        try {
                            // Take all the eventInfo args, and prefix with the viewmodel
...
ko.bindingHandlers['submit'] = {
    'init': function (element, valueAccessor, allBindingsAccessor, viewModel) {
        ...
        ko.utils.registerEventHandler(element, "submit", function (eventInfo) {
            var handlerReturnValue;
...
ko.bindingHandlers['value'] = {
    'init': function (element, valueAccessor, allBindingsAccessor) {
        // Always catch "change" eventInfo; possibly other events too if asked
        var eventsToCatch = ["change"];
        var requestedEventsToCatch = allBindingsAccessor()["valueUpdate"];
        if (requestedEventsToCatch) {
            if (typeof requestedEventsToCatch == "string") // Allow both individual eventInfo names, and arrays of eventInfo names
        ...
        ko.utils.arrayForEach(eventsToCatch, function(eventName) {
            // The syntax "after<eventname>" means "run the handler asynchronously after the eventInfo"
            // This is useful, for example, to catch "keydown" events after the browser has updated the control
            // (otherwise, ko.selectExtensions.readValue(this) will receive the control's value *before* the key eventInfo)
